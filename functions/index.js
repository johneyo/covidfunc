...
const dayjs = __webpack_require__(804)
...
try {
    ...
    iconv_package = 'iconv';
    Iconv = __webpack_require__(716).Iconv;
} catch (E) {
...
var bomHandling = __webpack_require__(0),
...
iconv.getCodec = function getCodec(encoding) {
    if (!iconv.encodings)
        iconv.encodings = __webpack_require__(417); // Lazy load all encoding definitions.
...
if (nodeVer) {
    ...
    if (nodeVerArr[0] > 0 || nodeVerArr[1] >= 10) {
        __webpack_require__(873)(iconv);
    }

    // Load Node primitive extensions.
    __webpack_require__(983)(iconv);
}
...
const fetch = __webpack_require__(197)
const credentials = __webpack_require__(679)
...
const dsv = __webpack_require__(101)
...
module.exports = {
    ...
    'shiftjis': {
        type: '_dbcs',
        table: function() { return __webpack_require__(973) },
        encodeAdd: {'\u00a5': 0x5C, '\u203E': 0x7E},
    ...
    'eucjp': {
        type: '_dbcs',
        table: function() { return __webpack_require__(558) },
        encodeAdd: {'\u00a5': 0x5C, '\u203E': 0x7E},
    },
    ...
    'cp936': {
        type: '_dbcs',
        table: function() { return __webpack_require__(763) },
    },

    // GBK (~22000 chars) is an extension of CP936 that added user-mapped chars and some other.
    'gbk': {
        type: '_dbcs',
        table: function() { return __webpack_require__(763).concat(__webpack_require__(728)) },
    },
    'xgbk': 'gbk',
    'isoir58': 'gbk',
    ...
    'gb18030': {
        type: '_dbcs',
        table: function() { return __webpack_require__(763).concat(__webpack_require__(728)) },
        gb18030: function() { return __webpack_require__(255) },
        encodeSkipVals: [0x80],
        encodeAdd: {'€': 0xA2E3},
    },
    ...
    'cp949': {
        type: '_dbcs',
        table: function() { return __webpack_require__(737) },
    },
    ...
    'big5hkscs': {
        type: '_dbcs',
        table: function() { return __webpack_require__(280).concat(__webpack_require__(303)) },
...
const parseCsv = __webpack_require__(261);
const getSupplement = __webpack_require__(947);
const mergeData = __webpack_require__(31);
const prepareScrapedData = __webpack_require__(190);
const deleteAllFeatures = __webpack_require__(772);
const uploadToDataHub = __webpack_require__(240);
const credentials = __webpack_require__(679);
...
exports.handler = (event, context, callback) => {
  ...
  const sheetQueries = sheets.map((sheetName) => {
    return fetch(
      `https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_${sheetName}_global.csv`
    ).then((d) => d.text());
  });
  ...
  Promise.all([
    ...
    const [confirmedRaw, recoveredRaw, deathRaw] = values
      .slice(0, 3)
      .map((d) => parseCsv(d));
    const dxyData = prepareScrapedData(values[3]);

    const [dateHeadersRaw, otherHeaders] = confirmedRaw.columns.reduce(
      (acc, cur, i) => {
        if (!i) acc = [[], []];
    ...
    const dateHeaders = dateHeadersRaw.map((d) =>
      dayjs(d).format("M/D/YYYY h:mm a")
    );
    ...
    const jhuData = confirmed.map((d) => {
      ...
      const supplementRecovered = getSupplement(
        "recoveries_",
        d,
        recovered,
        dateHeaders
      ...
      features: mergedData.map((properties) => {
        ...
        return {
          ...
          geometry: {
            type: "Point",
            coordinates: [
              parseFloat(properties.long),
              parseFloat(properties.lat),
            ],
          },
          properties: newProps,
        };
